...
`).join('');
    ...
    createProductData(websiteId, orderCode, status, price) {
        ...
        return {
            ...
            type: "Birthday",
            price: price,
            images: "./uploads/birthday-20250708-072056-bolsw2.png",
...
function initializePricingCalculator() {
 ...
 // Book page flip interaction
document.addEventListener('DOMContentLoaded', () => {
    const book = document.getElementById('book');
    const bookContainer = document.querySelector('.book-container');
    const giftImage = document.getElementById('gift-image');

    if (!book || !bookContainer || !giftImage) return;

    // Function to initialize page flip interaction
    ...
    // Listen for when giftImage is shown (display != none)
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.attributeName === 'style') {
                const display = window.getComputedStyle(giftImage).display;
                if (display !== 'none') {
                    // Delay showing book container until after happy3.gif is fully visible for 3 seconds
                    setTimeout(() => {
                        // Show book container
                        ...
                        book.style.transform = 'scale(1) translateY(0px)';
                        book.style.transition = 'opacity 0.8s, transform 0.8s';

                        // Initialize page flip interaction
                        initPageFlip();
    ...
    observer.observe(giftImage, { attributes: true });
});

// Remove pricingContainer element if exists
document.addEventListener('DOMContentLoaded', () => {
    const pricingContainer = document.getElementById('pricingContainer');
    if (pricingContainer) {
        pricingContainer.remove();
    }
});
